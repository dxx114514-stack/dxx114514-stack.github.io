  ...
  if (option.effects && typeof option.effects === 'object') {
    Object.entries(option.effects).forEach(([key, value]) => {
      if (key === 'random') {
        ...
        playerStats[randomStat] = Math.max(0, (playerStats[randomStat] || 0) + 1);
        ...
        playerStats[key] = Math.max(0, playerStats[key] + value);
      }
    });
...
// ========== 作弊系统 ==========
let cheatTitleClicks = 0;
let timePointClicks = 0;

const cheatAttributes = [
  { id: 'player-determination', stat: 'determination', label: '决心' },
  { id: 'player-mood', stat: '_mood', label: '心态' },
  { id: 'player-dp', stat: 'dp', label: '动态规划' },
  { id: 'player-ds', stat: 'ds', label: '数据结构' },
  { id: 'player-string', stat: 'string', label: '字符串' },
  { id: 'player-graph', stat: 'graph', label: '图论' },
  { id: 'player-combinatorics', stat: 'combinatorics', label: '组合计数' },
  { id: 'player-thinking', stat: 'thinking', label: '思维' },
  { id: 'player-coding', stat: 'coding', label: '代码' },
  { id: 'player-carefulness', stat: 'carefulness', label: '细心' },
  { id: 'player-quickness', stat: 'quickness', label: '迅捷' },
  { id: 'player-mental', stat: 'mental', label: '心理素质' },
  { id: 'player-culture', stat: 'culture', label: '文化课' }
];

function initCheatSystem() {
  const titleEl = document.querySelector('#player-stats-panel h3');
  if (!titleEl) return;
  titleEl.style.cursor = 'pointer';
  titleEl.addEventListener('click', () => {
    cheatTitleClicks++;
    if (cheatTitleClicks >= 10) {
      cheatTitleClicks = 0;
      openCheatEditor();
    }
  });
  ...
  // 剩余时间点连点作弊
  const timeEl = document.getElementById('time-points');
  if (timeEl) {
    timeEl.parentElement.style.cursor = 'pointer';
    timeEl.parentElement.addEventListener('click', () => {
      timePointClicks++;
      if (timePointClicks >= 10) {
        timePointClicks = 0;
        openTimePointEditor();
      }
    });
...
function openCheatEditor() {
  const list = document.getElementById('cheat-list');
  list.innerHTML = '';
  cheatAttributes.forEach(attr => {
    const value = attr.stat === '_mood' ? mood : (playerStats[attr.stat] || 0);
    const item = document.createElement('div');
    item.className = 'cheat-item';
    item.innerHTML = `
      <label>${attr.label}</label>
      <input type="number" id="cheat-input-${attr.stat}" value="${value}">
    `;
...
function saveAllCheats() {
  cheatAttributes.forEach(attr => {
    const input = document.getElementById('cheat-input-' + attr.stat);
    let value = parseInt(input.value);
    if (isNaN(value)) return;
    if (attr.stat === '_mood') {
      mood = value;
    } else {
      playerStats[attr.stat] = value;
    }
    logEvent(`[作弊] ${attr.label} 已修改为 ${value}`, 'event');
  });
...
// 时间点作弊
function openTimePointEditor() {
  const list = document.getElementById('cheat-list');
  list.innerHTML = `
    <div class="cheat-item">
      <label>剩余时间点</label>
      <input type="number" id="cheat-timepoint-input" value="${timePoints}">
    </div>
  `;
  // 临时替换保存按钮的行为
  const saveBtn = document.querySelector('.cheat-buttons .btn.green');
  saveBtn.textContent = '保存';
  saveBtn.onclick = saveTimePoint;
  document.getElementById('cheat-modal').style.display = 'flex';
  document.getElementById('cheat-timepoint-input').focus();
}

function saveTimePoint() {
  const input = document.getElementById('cheat-timepoint-input');
  const value = parseInt(input.value);
  if (isNaN(value)) return;
  timePoints = value;
  logEvent(`[作弊] 剩余时间点已修改为 ${value}`, 'event');
  document.getElementById('cheat-modal').style.display = 'none';
  // 恢复保存按钮
  const saveBtn = document.querySelector('.cheat-buttons .btn.green');
  saveBtn.textContent = '全部保存';
  saveBtn.onclick = saveAllCheats;
  updateStatus();
}
...
window.addEventListener('load', () => {
  initCheatSystem();
  document.getElementById('cheat-modal').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') saveAllCheats();
    if (e.key === 'Escape') cancelCheat();
  });
});